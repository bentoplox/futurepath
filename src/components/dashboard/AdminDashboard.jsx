...
const AdminDashboard = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ RESET LOGIC: Mark feedback as reviewed when Quality Control tab is opened
  ...
  const markFeedbackAsReviewed = async () => {
      try {
          ...
          // Optionally refresh summary stats to show 0 unread
          const sRes = await fetch('http://127.0.0.1:5000/api/admin/summary-stats');
          const sData = await sRes.json();
          if (sData.success) setSummaryStats(sData.stats);
  ...
  // Helper for rendering Sub-Navigation (Centered)
  ...
  return (
    <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: fontStack, margin: '-20px', paddingBottom: '80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', position: 'relative', zIndex: 1, maxWidth: '1400px', margin: '0 auto 40px auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.7)', fontWeight: '600' }}>Administrator</div>
                </div>
      ...
      <div style={{ maxWidth: '1400px', margin: '-60px auto 0 auto', padding: '0 40px', position: 'relative', zIndex: 10 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            {activeSubTab === 'overview' && (
              <div style={{ width: '100%', boxSizing: 'border-box' }}>
                 {/* 5-BLOCK KPI SCORECARD: SINGLE BALANCED ROW */}
                 <div style={{ 
                    display: 'grid', 
                    ...
                    <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0', textAlign: 'center', fontFamily: fontStack }}>
                        ...
                        <div style={{ fontSize: '34px', fontWeight: '800', color: '#10b981' }}>{summaryStats.verified_alumni}</div>
                    </div>
                    <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0', textAlign: 'center', fontFamily: fontStack }}>
                        ...
                        <div style={{ fontSize: '34px', fontWeight: '800', color: '#f59e0b' }}>{summaryStats.pending_moderation}</div>
                    </div>
                    <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0', textAlign: 'center', fontFamily: fontStack }}>
                    ...
                    <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0', textAlign: 'center', fontFamily: fontStack }}>
                        ...
                        <div style={{ fontSize: '34px', fontWeight: '800', color: '#ef4444' }}>{summaryStats.unread_student_reports}</div>
                    </div>
                 </div>
              </div>
            )}
            ...
            {activeSubTab === 'advisor' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                 <AIFacultyAdvisor recommendations={aiRecommendations} setRecommendations={setAiRecommendations} />
              </div>
            )}
                                      ...
                                      {skillHeatmapData.filter(s => s.career_name === cName).map((row, i) => (
                                          <React.Fragment key={i}>
                                              ...
                                              {[row.y1, row.y2, row.y3, row.y4].map((val, idx) => (
                                                  <div key={idx} style={{ background: getHeatmapColor(val), color: val === 0 ? '#94a3b8' : 'white', padding: '18px', textAlign: 'center', fontWeight: '800', fontSize: '14px' }}>{val === 0 ? '-' : `${val}%`}</div>
                                              ))}
                                          </React.Fragment>
                                      ))}
                                  </div>
            ...
            {activeSubTab === 'tracer' && (
              <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '30px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0' }}>
        ...
        {activeTab === 'curriculum' && (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {renderSubNav([
              ...
              { id: 'pathways', label: '🛤️ Learning Pathways' }
            ])}
        ...
        {/* --- TAB 3: CONTENT QUALITY CONTROL (Vertical Stack) --- */}
        {activeTab === 'quality' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
            
            {/* Top: Quality Assurance Hub */}
            <div style={{ backgroundColor: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.02)', border: '1px solid #e2e8f0' }}>
                <div style={{ marginBottom: '30px' }}>
                    <h3 style={{ fontSize: '22px', fontWeight: '800', color: brandPurple, marginBottom: '10px' }}>⚖️ Quality Assurance Hub</h3>
                    <p style={{ fontSize: '15px', color: '#64748b' }}>Review crowdsourced flags and maintain academic integrity across all modules.</p>
                </div>
                <AdminQualityControl umBlue={umBlue} umLightBlue={umLightBlue} />
            </div>

            {/* Bottom: Critical Student Flags (Full Width) */}
            <div style={{ backgroundColor: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.02)', border: '1px solid #e2e8f0' }}>
                 <div style={{ marginBottom: '30px' }}>
                    <h4 style={{ fontSize: '20px', fontWeight: '800', color: '#1e293b', marginBottom: '10px' }}>⚠️ Critical Student Flags</h4>
                    <p style={{ fontSize: '15px', color: '#64748b' }}>Real-time reports from the student cohort regarding dead links or outdated concepts.</p>
                 </div>
                 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    {feedback.length > 0 ? feedback.slice(0, 6).map((f, i) => (
                      <div key={i} style={{ padding: '20px', borderRadius: '16px', background: '#fff1f2', border: '1px solid #fecaca', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <div style={{ fontSize: '14px', fontWeight: '800', color: '#be123c' }}>{f.skill_name}</div>
                          <div style={{ fontSize: '12px', color: '#e11d48', marginTop: '4px' }}>Issue: {f.reason || "Content flag reported"}</div>
                        </div>
                        <span style={{ backgroundColor: '#be123c', color: 'white', padding: '6px 14px', borderRadius: '10px', fontSize: '11px', fontWeight: '800' }}>URGENT</span>
                      </div>
                    )) : (
                        <div style={{ gridColumn: 'span 2', textAlign: 'center', padding: '40px', color: '#64748b', fontStyle: 'italic' }}>
                            No active student flags detected.
                        </div>
                    )}
                 </div>
            </div>
        ...
        {activeTab === 'moderation' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '1000px', margin: '0 auto' }}>
              <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '16px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #e2e8f0' }}>
                ...
                <span style={{ backgroundColor: '#f1f5f9', color: '#64748b', padding: '6px 14px', borderRadius: '10px', fontSize: '12px', fontWeight: '700' }}>
                  {posts.filter(p => p.status === 'pending').length} Pending Reviews
                </span>
              </div>
              ...
              {posts.filter(p => p.status === 'pending').map(post => (
                  <div key={post.id} style={{ backgroundColor: 'white', padding: '30px', borderRadius: '20px', borderLeft: `6px solid ${getTypeColor(post.post_type)}`, boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                          <div>
                              <span style={{ background: getTypeColor(post.post_type), color: 'white', padding: '4px 14px', borderRadius: '20px', fontSize: '10px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{post.post_type.replace('_', ' ')}</span>
                          ...
                          <div style={{ display: 'flex', gap: '12px' }}>
                              <button onClick={() => updateStatus(post.id, 'approved')} style={{ padding: '12px 24px', background: '#dcfce7', color: '#15803d', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: 'pointer', fontSize: '13px', transition: 'all 0.2s' }}>✅ Approve</button>
                              <button onClick={() => updateStatus(post.id, 'rejected')} style={{ padding: '12px 24px', background: '#fee2e2', color: '#b91c1c', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: 'pointer', fontSize: '13px', transition: 'all 0.2s' }}>❌ Reject</button>
                          </div>
                      </div>
                      ...
                      <div style={{ color: '#334155', lineHeight: '1.7', background: '#f8fafc', padding: '25px', borderRadius: '16px', border: '1px solid #f1f5f9', fontSize: '15px' }}>{post.content}</div>
                  </div>
              ))}
