  ...
  return (
    <div style={{ backgroundColor: '#f9f9f9', minHeight: '100vh', fontFamily: "'Aeonik', 'Plus Jakarta Sans', sans-serif", margin: '-20px', paddingBottom: '50px' }}>
      ...
      <div style={{ 
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontSize: '42px', margin: '0 0 10px 0', fontWeight: '700' }}>
            {activeTab === 'skills' ? 'Skills Gap' : activeTab === 'employability' ? 'Employability' : activeTab === 'moderation' ? 'Content Moderation' : activeTab === 'content' ? 'Content Engine' : activeTab === 'resources' ? 'Resource Library' : activeTab === 'quizzes' ? 'Assessment Bank' : 'System Overview'}
          </h1>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
        ...
        {/* === TAB: SKILLS GAP (Grouped Heatmap Matrix) === */}
        {activeTab === 'skills' && (
            <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '30px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '30px', borderBottom: '1px solid #f3f4f6', paddingBottom: '20px' }}>
                    <div>
                        ...
                        <p style={{ color: '#6b7280', fontSize: '14px', marginTop: '5px' }}>Analyze competency scores and qualitative feedback.</p>
                    </div>
                ...
                {viewMode === 'matrix' ? (
                        ...
                        {[...new Set(skillHeatmapData.map(s => s.career_name))].map(cName => (
                            <div key={cName} style={{ border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', gap: '1px', background: '#e5e7eb' }}>
                                    <div style={{ background: '#f9fafb', padding: '12px', fontWeight: 'bold', fontSize: '12px', color: '#4b5563' }}>TECHNICAL SKILL</div>
                                    {['Y1', 'Y2', 'Y3', 'Y4'].map(y => <div key={y} style={{ background: '#f9fafb', padding: '12px', textAlign: 'center', fontWeight: 'bold', fontSize: '12px', color: '#4b5563' }}>{y}</div>)}
                                        ...
                                        <React.Fragment key={i}>
                                            ...
                                            {[row.y1, row.y2, row.y3, row.y4].map((val, idx) => (
                                                <div key={idx} style={{ 
                                                    ...
                                                    color: val === 0 ? '#9ca3af' : 'white', 
                                                    padding: '15px', textAlign: 'center', fontWeight: '800', fontSize: '14px'
                                                }}>{val === 0 ? '-' : `${val}%`}</div>
                                            ))}
                                        </React.Fragment>
                                    ))}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {feedback.map((f, i) => (
                            <div key={i} style={{ padding: '20px', borderRadius: '10px', background: '#f8fafc', borderLeft: `5px solid ${umLightBlue}`, boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                    <strong style={{ color: umBlue }}>{f.skill_name}</strong>
                                    <span style={{ fontSize: '11px', color: '#9ca3af', fontWeight: 'bold' }}>{new Date(f.created_at).toLocaleDateString()}</span>
                                </div>
                                <p style={{ margin: 0, fontStyle: 'italic', color: '#4b5563' }}>"{f.reason}"</p>
                            </div>
                        ))}
                    </div>
                )}
        ...
        {/* === TAB: MODERATION (Premium High-Fidelity Cards) === */}
        {activeTab === 'moderation' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '900px', margin: '0 auto' }}>
                {posts.filter(p => p.status === 'pending').map(post => (
                    <div key={post.id} style={{ backgroundColor: 'white', padding: '30px', borderRadius: '15px', borderLeft: `6px solid ${getTypeColor(post.post_type)}`, boxShadow: '0 10px 15px -3px rgba(0,0,0,0.05)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                            <div>
                                ...
                                <h3 style={{ margin: '12px 0 5px 0', fontSize: '22px', fontWeight: '700', color: '#111827' }}>{post.title}</h3>
                        ...
                        {post.company_name && <div style={{ background: '#f8fafc', padding: '8px 15px', borderRadius: '8px', fontSize: '13px', fontWeight: '700', color: umBlue, display: 'inline-flex', alignItems: 'center', gap: '8px', marginBottom: '15px' }}>🏢 {post.company_name}</div>}
                        <div style={{ color: '#374151', lineHeight: '1.6', background: '#f9fafb', padding: '20px', borderRadius: '10px', fontSize: '15px' }}>{post.content}</div>
                    </div>
                ))}
