...
const ResourceManager = ({ supabase, umBlue, umLightBlue, umGold }) => {
    ...
    const saveApprovedQuiz = async () => {
        ...
        if (res.ok) {
            alert("Quizzes saved to Assessment Bank!");
            setQuizDraft(null);
        }
    };

    const handleDeleteSkill = async (skill) => {
        if (!window.confirm(`Delete "${skill.skill_name}" (Step ${skill.step_order})?`)) return;
        const res = await fetch(`http://127.0.0.1:5000/api/admin/career/delete-skill?career_id=${expandedCareer}&skill_id=${skill.skill_id}&step_order=${skill.step_order}`, {
            method: 'DELETE'
        });
        if (res.ok) {
            alert("Skill removed and roadmap re-ordered.");
    ...
    const handleReorder = async (skill, direction) => {
        const res = await fetch('http://127.0.0.1:5000/api/admin/career/reorder-skill', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                career_id: expandedCareer,
                skill_id: skill.skill_id,
                step_order: skill.step_order,
                direction: direction
            })
        });
        if (res.ok) {
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: '30px', backgroundColor: 'white', borderRadius: '12px', padding: '30px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', minHeight: '600px' }}>
            {/* Sidebar */}
                ...
                {careerMap.map(career => (
                    <div key={career.career_id} style={{ marginBottom: '10px' }}>
                        ...
                        {expandedCareer === career.career_id && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginLeft: '25px', marginTop: '5px', borderLeft: '2px solid #e5e7eb', paddingLeft: '10px' }}>
                                ...
                                {career.skills.map((skill, idx) => (
                                    <div key={skill.skill_id} style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                                        {/* Reorder Buttons */}
                                        ...
                                        <button onClick={() => { setSelectedSkill(skill); fetchVerified(skill.concept_tag); setQuizDraft(null); }} style={{ flex: 1, textAlign: 'left', padding: '10px 12px', borderRadius: '8px', border: 'none', fontSize: '13px', cursor: 'pointer', background: selectedSkill?.skill_id === skill.skill_id ? '#eff6ff' : 'transparent', color: selectedSkill?.skill_id === skill.skill_id ? umLightBlue : '#4b5563', fontWeight: selectedSkill?.skill_id === skill.skill_id ? '800' : '500', display: 'flex', justifyContent: 'space-between' }}>
                                            <span style={{ maxWidth: '100px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{skill.skill_name}</span>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {verifiedResources.map(res => (
                                    <div key={res.resource_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', border: '1px solid #e5e7eb', borderRadius: '12px', backgroundColor: 'white' }}>
                                        <div><strong>{res.title}</strong><div style={{ fontSize: '13px', color: '#6b7280' }}>{res.provider} • <a href={res.url} target="_blank" rel="noreferrer" style={{ color: umLightBlue }}>Visit ↗</a></div></div>
                                        <button onClick={async () => { if (window.confirm("Remove?")) { await fetch(`http://127.0.0.1:5000/api/admin/resources/delete/${res.resource_id}`, { method: 'DELETE' }); fetchVerified(selectedSkill.concept_tag); } }} style={{ color: '#ef4444', background: 'none', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>Remove</button>
                                    </div>
                                ))}
                            </div>
                        </div>

                        <div style={{ backgroundColor: '#f8fafc', padding: '35px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
